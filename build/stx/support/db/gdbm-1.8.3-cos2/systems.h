...
/* Vanilla replacement; you have to wait for a full implementation... */
...
#if !HAVE_FSYNC
#ifdef WIN32
#define fsync(f) (FlushFileBuffers((HANDLE)_get_osfhandle(f)) ? 0 : -1)
#else
...
#if !HAVE_LINK
#ifdef WIN32
/* Don't do a real link(), but rather a blunt file copy. */
#define link(src, dest) (CopyFile(src, dest, TRUE) ? 0 : -1)
#endif
/* No idea what to do on a non-Windows system... */
#endif
