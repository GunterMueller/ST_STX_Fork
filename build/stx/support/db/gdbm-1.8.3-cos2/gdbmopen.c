...
/* O_BINARY is pre-defined on non-POSIX systems that handle text and binary
   files differently. */
#ifndef O_BINARY
#define O_BINARY 0
#endif
     ...
     void (*fatal_func) ();
{
  ...
  switch (flags & GDBM_OPENMASK)
    {
      case GDBM_READER:
	dbf->desc = open (dbf->name, O_BINARY|O_RDONLY, 0);
	break;

      case GDBM_WRITER:
	dbf->desc = open (dbf->name, O_BINARY|O_RDWR, 0);
	break;

      case GDBM_NEWDB:
	dbf->desc = open (dbf->name, O_BINARY|O_RDWR|O_CREAT, mode);
	need_trunc = TRUE;
	break;
