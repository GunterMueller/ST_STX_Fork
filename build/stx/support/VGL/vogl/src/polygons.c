...
/*
 * software pipelined version of corresponding
 * code in matrix.c
 */
	...
	float	x, y, z;
{
	...
	if (vdevice.inpolygon) {
	    vwarning("pmv: already inPolygon");
	}
#endif
...
bgnpolygon()
{
	if (vdevice.bgnmode != NONE) {
		verror("vogl: bgnpolygon already in some other bgn routine");
		return;
	}

	if (vdevice.inpolygon) {
	    verror("vogl: bgnpolygon: already inPolygon");
	    return;
	}
...
{
#ifndef ORIGINAL
        if (vdevice.bgnmode != VPOLY) {
            vwarning("vogl: endpolygon, without bgnpolygon (%d)",
                            vdevice.bgnmode);
            if (vdevice.bgnmode == NONE) {
                return;
            }
	    ...
	    if (! vdevice.inpolygon) {
	        vwarning("vogl: endpolygon: not inPolygon");
	    }
	}
#endif
...
{
	if (vdevice.bgnmode != NONE) {
		verror("vogl: bgntmesh already in some other bgn routine");
		return;
	}
	if (vdevice.inpolygon) {
	    verror("vogl: bgntmesh: already inPolygon");
	    return;
	}
...
endtmesh()
{
#ifndef ORIGINAL
        if (vdevice.bgnmode != VTMESH) {
            vwarning("vogl: endtmesh, without bgntmesh (%d)",
                            vdevice.bgnmode);
            if (vdevice.bgnmode == NONE) {
                return;
            }
	    ...
	    if (! vdevice.inpolygon) {
	        vwarning("vogl: endtmesh: not inPolygon");
	    }
	}
#endif
...
{
	if (vdevice.bgnmode != NONE) {
		verror("vogl: bgnqstrip already in some other bgn routine");
		return;
	}
	if (vdevice.inpolygon) {
	    verror("vogl: bgnqstrip: already inPolygon");
	    return;
	}
...
{
#ifndef ORIGINAL
        if (vdevice.bgnmode != VQSTRIP) {
            vwarning("vogl: endqstrip, without bgnqstrip (%d)",
                            vdevice.bgnmode);
            if (vdevice.bgnmode == NONE) {
                return;
            }
	    ...
	    if (! vdevice.inpolygon) {
	        vwarning("vogl: endqstrip: not inPolygon");
	    }
	}
#endif
