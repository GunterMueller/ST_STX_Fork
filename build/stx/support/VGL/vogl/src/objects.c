	...
	long	ob;
{
...
#ifndef ORIGINAL
	if (vdevice.inobject) {
		verror("makeobj: already in makeobj");
		return;
	}
#endif
	...
	for (o = object_table[n % MAXENTS]; o != (VObject *)NULL; o = o->next)
		if (o->obno == n) {
                        if (o->beingCalled) {
                            verror("VGL: *** cannot make object while its being called");
                            return;
                        }
...
closeobj()
{
	...
	o->next = object_table[current_obno % MAXENTS];

	object_table[current_obno % MAXENTS] = o;
	...
	long	ob;
{
	...
	if (o != (VObject *)NULL) {
		if (o->beingCalled) {
		    verror("VGL: *** cannot delete object while its being called");
		    return;
		}
		...
		while (t != et) {
			...
			case SCALE:
...

				{
				    ...
				    tf = t[1].f;
				    vdevice.transmat->m[0][0] *= tf;
				    vdevice.transmat->m[0][1] *= tf;
				    vdevice.transmat->m[0][2] *= tf;
				    vdevice.transmat->m[0][3] *= tf;

				    tf = t[2].f;
				    vdevice.transmat->m[1][0] *= tf;
				    vdevice.transmat->m[1][1] *= tf;
				    vdevice.transmat->m[1][2] *= tf;
				    vdevice.transmat->m[1][3] *= tf;

				    tf = t[3].f;
				    vdevice.transmat->m[2][0] *= tf;
				    vdevice.transmat->m[2][1] *= tf;
				    vdevice.transmat->m[2][2] *= tf;
				    vdevice.transmat->m[2][3] *= tf;
				}
				t += 4;
				break;
			case ROTATE:
