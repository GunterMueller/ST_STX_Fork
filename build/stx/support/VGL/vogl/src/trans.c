...
#else
        top = vdevice.transmat;

        top->m[3][0] += x * top->m[0][0]
                      + y * top->m[1][0]
                      + z * top->m[2][0];

        top->m[3][1] += x * top->m[0][1]
                      + y * top->m[1][1]
                      + z * top->m[2][1];

        top->m[3][2] += x * top->m[0][2]
                      + y * top->m[1][2]
                      + z * top->m[2][2];

        top->m[3][3] += x * top->m[0][3]
                      + y * top->m[1][3]
                      + z * top->m[2][3];
#endif
}
        ...
        char    axis;
{
        ...
        case 'X':
                tmp = top->m[1][0];
                top->m[1][0] = costheta * tmp
                                          + sintheta * top->m[2][0];
                top->m[2][0] = costheta * top->m[2][0]
                                          - sintheta * tmp;

                tmp = top->m[1][1];
                top->m[1][1] = costheta * tmp
                                          + sintheta * top->m[2][1];
                top->m[2][1] = costheta * top->m[2][1]
                                          - sintheta * tmp;
                tmp = top->m[1][2];
                top->m[1][2] = costheta * tmp
                                          + sintheta * top->m[2][2];
                top->m[2][2] = costheta * top->m[2][2]
                                          - sintheta * tmp;

                tmp = top->m[1][3];
                top->m[1][3] = costheta * tmp
                                          + sintheta * top->m[2][3];
                top->m[2][3] = costheta * top->m[2][3]
                                          - sintheta * tmp;
                break;
        case 'y':
        case 'Y':
                tmp = top->m[0][0];
                top->m[0][0] = costheta * tmp
                                          - sintheta * top->m[2][0];
                top->m[2][0] = sintheta * tmp
                                          + costheta * top->m[2][0];
                tmp = top->m[0][1];
                top->m[0][1] = costheta * tmp
                                          - sintheta * top->m[2][1];
                top->m[2][1] = sintheta * tmp
                                          + costheta * top->m[2][1];
                tmp = top->m[0][2];
                top->m[0][2] = costheta * tmp
                                          - sintheta * top->m[2][2];
                top->m[2][2] = sintheta * tmp
                                          + costheta * top->m[2][2];
                tmp = top->m[0][3];
                top->m[0][3] = costheta * tmp
                                          - sintheta * top->m[2][3];
                top->m[2][3] = sintheta * tmp
                                          + costheta * top->m[2][3];
                break;
        case 'z':
        case 'Z':
                ...
                top->m[0][0] = costheta * tmp
                                          + sintheta * top->m[1][0];
                top->m[1][0] = costheta * top->m[1][0]
                                          - sintheta * tmp;

                tmp = top->m[0][1];
                top->m[0][1] = costheta * tmp
                                          + sintheta * top->m[1][1];
                top->m[1][1] = costheta * top->m[1][1]
                                          - sintheta * tmp;

                tmp = top->m[0][2];
                top->m[0][2] = costheta * tmp
                                          + sintheta * top->m[1][2];
                top->m[1][2] = costheta * top->m[1][2]
                                          - sintheta * tmp;

                tmp = top->m[0][3];
                top->m[0][3] = costheta * tmp
                                          + sintheta * top->m[1][3];
                top->m[1][3] = costheta * top->m[1][3]
                                          - sintheta * tmp;
                break;
        default:
                verror("rot: illegal axis of rotation");
