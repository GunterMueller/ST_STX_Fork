...
GLboolean APIENTRY fxMesaSelectCurrentBoard(int n)
{
  fxQueryHardware();
...
fxMesaContext APIENTRY fxMesaGetCurrentContext(void)
{
  return fxMesaCurrentCtx;
}


void APIENTRY fxMesaSetNearFar(GLfloat n, GLfloat f)
{
  if(fxMesaCurrentCtx)
 ...
 */
void APIENTRY glGlobalTextureLODBiasFXMESA(GLfloat biasVal)
{
...
void APIENTRY gl3DfxSetPaletteEXT(GLuint *pal)
{
  ...
  if(fxMesaCurrentCtx) {
    fxMesaCurrentCtx->haveGlobalPaletteTexture=1;

    grTexDownloadTable(GR_TMU0,GR_TEXTABLE_PALETTE,(GuTexPalette *)pal);

    if(fxMesaCurrentCtx->haveTwoTMUs)
      grTexDownloadTable(GR_TMU1,GR_TEXTABLE_PALETTE,(GuTexPalette *)pal);
  }
}
...
fxMesaContext APIENTRY fxMesaCreateBestContext(GLuint win,GLint width, GLint height,
 ...
 */
fxMesaContext APIENTRY fxMesaCreateContext(GLuint win,GrScreenResolution_t res,
					   ...
					   const GLint attribList[])
{
  ...
  if(verbose)
    fprintf(stderr,"Mesa fx Voodoo Device Driver v0.29\nWritten by David Bucciarelli (tech.hmw@plus.it)\n");
  ...
  fxMesa=(fxMesaContext)malloc(sizeof(struct tfxMesaContext));
  if(!fxMesa) {
#if defined(DEBUG_FXMESA)
  ...
  fxMesa->width=grSstScreenWidth();
  fxMesa->height=grSstScreenHeight();
  ...
  fxMesa->color=FXCOLOR(255,255,255,255);
  fxMesa->clearC=0;
  fxMesa->clearA=0;
  ...
  fxMesa->glCtx=gl_create_context(fxMesa->glVis,
 ...
 */
void APIENTRY fxMesaUpdateScreenSize(fxMesaContext fxMesa)
{
  fxMesa->width=grSstScreenWidth();
  fxMesa->height=grSstScreenHeight();
}
 ...
 */
void APIENTRY fxMesaDestroyContext(fxMesaContext fxMesa)
{
#if defined(DEBUG_FXMESA)
 ...
 */
void APIENTRY fxMesaMakeCurrent(fxMesaContext fxMesa)
{
#if defined(DEBUG_FXMESA)
  ...
  if(fxMesaCurrentCtx)
    grGlideGetState(&fxMesaCurrentCtx->state);

  fxMesaCurrentCtx=fxMesa;

  grSstSelect(fxMesa->board);
  grGlideSetState(&fxMesa->state);
 ...
 */
void APIENTRY fxMesaSwapBuffers(void)
{
#if defined(DEBUG_FXMESA)
  ...
  if(fxMesaCurrentCtx)
    if(fxMesaCurrentCtx->haveDoubleBuffer) {
       ...
       */
      while(grBufferNumPending()>fxMesaCurrentCtx->maxPendingSwapBuffers);
 ...
 */
int APIENTRY fxQueryHardware(void)
{
#if defined(DEBUG_FXMESA)
  ...
  if(!glbGlideInitialized) {
    grGlideInit();
    if(grSstQueryHardware(&glbHWConfig)) {
      grSstSelect(glbCurrentBoard);
      glb3DfxPresent=1;
 ...
 */
void APIENTRY fxCloseHardware(void)
{
  if(glbGlideInitialized) {
    if(getenv("MESA_FX_INFO")) {
      GrSstPerfStats_t		st;
