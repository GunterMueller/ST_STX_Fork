...
void fxDDTexUseGlbPalette(GLcontext *ctx, GLboolean state)
{
  ...
  if(state) {
    ...
    if(fxMesa->haveTwoTMUs)
      grTexDownloadTable(GR_TMU1,GR_TEXTABLE_PALETTE,&(fxMesa->glbPalette));
  } else {
    fxMesa->haveGlobalPaletteTexture=0;

    if((ctx->Texture.Set[0].Current==ctx->Texture.Set[0].Current2D) &&
       (ctx->Texture.Set[0].Current2D!=NULL)) {
			       ...
			       GLboolean *istranslate)
{
  ...
  fxTexGetInfo(image->Width,image->Height,NULL,NULL,NULL,NULL,&wscale,&hscale);
