/* $Id: xmesa4.c,v 1.1 1998-10-05 22:35:01 cg Exp $ */
...
/*
 * $Log: xmesa4.c,v $
 * Revision 1.1  1998-10-05 22:35:01  cg
 * *** empty log message ***
 *
 * Revision 3.3  1998/08/16 15:00:08  brianp
 * added optimized TRUEDITHER triangle functions (Terence Ripperda)
 *
 * Revision 3.2  1998/07/17 03:38:37  brianp
 * added struct timespec declaration to silence a compiler warning
 *
 * Revision 3.1  1998/03/28 02:03:36  brianp
 * fixed G++ warnings
 *
...
{									\
   ...
   for (i=0;i<len;i++) {						\
      GLdepth z = FixedToDepth(ffz);					\
      if (z < zRow[i]) {						\
         PACK_TRUEDITHER( pRow[i], LEFT + i, Y, FixedToInt(ffr), FixedToInt(ffg),		\
				FixedToInt(ffb) );			\
         zRow[i] = z;							\
      }									\
      ffr += fdrdx;  ffg += fdgdx;  ffb += fdbdx;			\
...
#define SETUP_CODE					\
   unsigned long p;					\
   PACK_TRUECOLOR( p, VB->Color[pv][0],			\
		 VB->Color[pv][1], VB->Color[pv][2] );
#define INNER_LOOP( LEFT, RIGHT, Y )			\
{							\
   GLint i, len = RIGHT-LEFT;				\
...
{									\
   ...
   PIXEL_TYPE *pixel = pRow;						\
   for (xx=LEFT;xx<RIGHT;xx++,pixel++) {				\
      PACK_TRUEDITHER( *pixel, xx, Y, FixedToInt(ffr), FixedToInt(ffg),	\
				FixedToInt(ffb) );			\
      ffr += fdrdx;  ffg += fdgdx;  ffb += fdbdx;			\
   }									\
}
...
{							\
   ...
   unsigned long p;					\
   PIXEL_TYPE *pixel = pRow;				\
   PACK_TRUEDITHER( p, LEFT, Y, VB->Color[pv][0],	\
		 VB->Color[pv][1], VB->Color[pv][2] );	\
   for (xx=LEFT;xx<RIGHT;xx++,pixel++) {		\
      *pixel = p;					\
   }							\
