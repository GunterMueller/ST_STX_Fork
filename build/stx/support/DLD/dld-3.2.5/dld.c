...
#ifndef ORIGINAL
int dld_ignore_redefinitions = 0;
#endif
...
char *name;
{
	...
	if (olddef) {
		...
		if (dld_ignore_redefinitions) {
		    printf("DLD: ignore new definition of %s (used statically only)\n", name);
		    return;
		}
	    }
	    printf("multiple definition of %s (cannot resolve conflicts)\n", name);
#endif
	    fatal (DLD_EMULTDEFS);
	} else {
	    sp->defined = type;
