    ...
    char *ldname, *sym;
{
    ...
    D(printf("dl_getsymbol: nlist type:%x value:%x name:<%s> (<%s>)\n",
		nl[0].n_type, nl[0].n_value, nl[0].n_name, sym));
    if ( nl[0].n_type == 0) {
        D(printf("dl_getsymbol: \'%s\' not found in \'%s\'\n", sym, ldname));
        dl_error("No such symbol in binary: %s", sym);
        return 0;
    }
    D(printf("dl_getsymbol: \'%s\' is %x in \'%s\'\n", 
				sym, nl[0].n_value, ldname));
    ...
    char *binary, *module, *rtn;
{
    ...
    return (dl_funcptr)nl[0].n_value;
#else
    return dl_getsymbol(ldname, rtn);
#endif
}
...
#ifdef DEBUG
    fprintf(stderr, "dl_loadmod_mult(%s, %s, ..)\n", binary, module);
#endif
    if ( dl_loadmod_only(binary, module, &ldname) == 0 )
      return 0;
    rv = nlist(ldname, nl);
...
#ifdef DEBUG
    fprintf(stderr, "dl_loadmod_only(%s, %s, %s)\n", binary, module, *ldnamep);
#endif
