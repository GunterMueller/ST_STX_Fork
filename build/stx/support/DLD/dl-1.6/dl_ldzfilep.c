...
#if defined(__ultrix) || defined(_AIX)
# if defined(__ultrix)
#  include <fcntl.h>
# endif
  /*
   * mmap will fail - we know that already
   */
  char *mmap();
# ifndef PROT_EXECUTE
#   define PROT_EXECUTE PROT_READ
# endif
#else
# include <sys/fcntl.h>
#endif
    ...
    char *fn;
{
    ...
    */
    ...
    D(printf("dl_ldzfilep: %d sections\n", nsect));
#ifdef SYMHEADER
    D(printf("dl_ldzfilep: version stamp=%d, 0x%x\n", SYMHEADER(ldptr).vstamp,
	     SYMHEADER(ldptr).vstamp));
#endif
    /*
    ...
    */
	...
	D(printf("dl_ldzfilep: section %d=%s, 0x%x:0x%x@0x%x type 0x%x\n", i,
